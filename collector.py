...
def load_config():
    ...
    try:
        ...
        with open(toml_path, "rb") as f:
            ...
            # For local, you might still use service_account.json
            # But we will prioritize the Environment Variable for GitHub compatibility
            creds = os.environ.get("GCP_SERVICE_ACCOUNT")
            return api_key, sheet_id, creds
    except:
        # 2. GitHub Actions Fallback
        return os.environ.get("API_KEY_DATA"), os.environ.get("SHEET_ID"), os.environ.get("GCP_SERVICE_ACCOUNT")
...
def fetch_and_save():
    if not all([API_KEY, SHEET_ID, GCP_JSON]):
        print("❌ Missing configuration. Ensure GCP_SERVICE_ACCOUNT secret is set.")
        return
